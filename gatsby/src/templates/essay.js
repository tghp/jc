...
export default function Essay({
  data: { wpPost, furtherReadingPostsDefault, furtherReadingPostsOverride },
  pageContext: { referenceCount, latexCount, hasPdf },
}) {
  const {
    title,
    slug,
  ...
  const [referencesAreaMeasureRef, { width: referencesAreaWidth }] =
    useMeasure();
  ...
  const [imagesReady, setImagesReady] = useState(
    !content || !content.match(/<img/g),
  );
  const [openIndexes, setOpenIndexes] = useState([]);

  const onScroll = () => {
    const mainContentWindowTop =
      mainContent.current.getBoundingClientRect().top;
  ...
  useEffect(() => {
    ...
    const gridRowsDesktop = Object.values(referenceContentRefs.current)
      ...
      .map((articleReferenceInline, i) => {
        ...
        const articleReferenceListItemHeight =
          articleReferenceListItem.clientHeight;
        const inlineReferenceOffset = Math.max(
          20,
          articleReferenceInline.offsetTop - articleReferenceListItemOffsets,
        );
        articleReferenceListItemOffsets +=
          inlineReferenceOffset + articleReferenceListItemHeight;
   ...
   */
  const furtherReadingPosts =
    furtherReadingPostsOverride.posts.length === 0
        ...
        <div className="single-essay__grid">
          <div className="single-essay__sidebar">
            <div className="single-essay__sidebar-title" ref={sidebarTitle}>
              {seriesName && <div>{`${seriesName} / Part ${partNumber}`}</div>}
              {title}
            </div>
            {hasPdf || audioUrl || videoUrl ? (
              <div className="single-essay__sidebar-media-links">
                ...
                {audioUrl && (
                  <a href={audioUrl} target={`_blank`} aria-label="Audio link">
                    <ImageAudioLink />
                  </a>
                )}
                {videoUrl && (
                  <a href={videoUrl} target={`_blank`} aria-label="Video link">
                    <ImageVideoLink />
                  </a>
                )}
              </div>
            ) : null}
            <TableOfContents navItems={toc} />
          </div>

          <div className="single-essay__header">
            {modified && (
              <div className="single-essay__header-update-date">
                Last updated: {getPostDate(modified)}
              </div>
            )}
            {date && (
              <div className="single-essay__header-publish-date">
                Published: {getPostDate(date)}
              </div>
            )}
            {seriesName && (
              <div className="single-essay__header-series">
                <div className="single-essay__header-series-name">Series</div>
              ...
              <div className="single-essay__extra-reading-comments post-comments">
                ...
                <div className="post-comments__options">
                  ...
                  {substackUrl && (
                    <a
                      href={substackUrl}
                      className="post-comments__options-system"
                      target="_blank"
                      rel="noreferrer"
                    >
                      Substack
            ...
            {nextSeriesPartObj && (
              <div className="single-essay__extra-reading-next-series">
                <HeadingWithLink
                  ...
                  linkTo={getPostPath(
                    nextSeriesPartObj.slug,
                    nextSeriesPartObj.date,
                  )}
                  linkArrow={true}
                />
          ...
          {!!referenceCount &&
            !!referenceRowSizesDesktop &&
            !!referenceRowSizesMobile && (
              <References
...
export const query = graphql`
  query (
    ...
    furtherReadingPostsDefault: allWpPost(
      filter: {
        categories: { nodes: { elemMatch: { slug: { in: $postCategories } } } }
      }
    ) {
      posts: nodes {
        slug
    ...
    furtherReadingPostsOverride: allWpPost(
      filter: { databaseId: { in: $furtherReadingPosts } }
    ) {
      posts: nodes {
        slug
        title
