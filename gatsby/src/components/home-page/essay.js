...
const Essay = ({
  post: { title, slug, date, excerpt, tghpjcExternalUrl: externalUrl },
}) => {
  const essayOutput = (
    <>
      <div className="category-essays__essay-title">{title}</div>
      <div
        className="category-essays__essay-excerpt"
        dangerouslySetInnerHTML={{ __html: excerpt }}
      />
      <div className="category-essays__essay-date">{getPostDate(date)}</div>
