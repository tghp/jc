...
const PostArchive = ({ posts }) => {
  const postsUniqueYears = [
    ...new Set(posts.map((post) => getPostDateFullYear(post.date))),
  ].sort((postA, postB) => postB - postA);
  const postsArchive = postsUniqueYears.map((year) =>
    posts.filter((post) => year === getPostDateFullYear(post.date)),
  );

  return postsUniqueYears.map((year, index) => (
